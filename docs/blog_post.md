...
|---|---|---|---|---|---|
| `llama-3.3-70b-versatile` | 6 | 12 | 100% | 83.3% | ~275 min |
| `llama-3.1-8b-instant` | 50 | 72 | 100% | 58.3% | ~75 min |

---

## Finding 1: 100% Validation Rate, 58% Pass Rate — These Are Different Things

Both runs validated 100% of generated records. Zero schema failures. The 8B run still scored 58.3% overall quality pass rate.
...
**The rule:** schema validation is a floor, not a ceiling. 100% validation rate with 42% pass rate means your generator reliably produces well-formed data that often fails at what it's supposed to do. Evaluate fitness separately from structure.

---

## Finding 2: career_changer Fails 2× More Often — and It's Structural, Not Random

Failure rates by writing template (8B model, 72 pairs):

| Template | Pairs | Failure Rate |
|---|---|---|
| `career_changer` | 14 | **64.3%** |
| `formal` | 14 | 50.0% |
| `achievement_focused` | 21 | 33.3% |
| `casual` | 12 | 33.3% |
| `technical` | 11 | **27.3%** |
...
| good | 0.826 | 1.000 | 60–80% |
| partial | 0.804 | 0.833 | 40–60% |
| poor | 0.634 | 0.600 | 20–40% |
| mismatch | **0.600** | **0.300** | < 20% |

The 70B model gets "mismatch" to 0.30 average overlap — outside the target (< 0.20) but directionally correct. The 8B model produces 0.60 average overlap for "mismatch" pairs. The model intended to generate a resume with almost no skill overlap and produced one that's a reasonable partial fit.
...
Failure rates by mode (8B model, 72 pairs):

| Failure Mode | Rate |
|---|---|
| `seniority_mismatch` | **19.4%** |
| `missing_core_skill` | 13.9% |
| `low_skills_overlap` | 12.5% |
| `experience_mismatch` | 2.8% |
| `hallucinated_skill` | 1.4% |
| `awkward_language` | **0.0%** |

Seniority mismatch (19.4%) nearly doubles the next failure mode. The same pattern holds in the 70B run (16.7%). Two models, consistent result — this is a generation behavior, not a model artifact.
...
|---|---|---|---|---|
| `llama-3.3-70b-versatile` | 12 | ~275 min | 83.3% | ~$0.54 |
| `llama-3.1-8b-instant` | 72 | ~75 min | 58.3% | ~$0.003 |

The 70B model produces 25% better pass rates. It also takes 23× longer per pair (275 min / 12 pairs vs. 75 min / 72 pairs) and costs ~180× more.

The 275 minutes is not inference time — it's rate-limit throttling on Groq's free tier. The 70B model triggers more aggressive rate limiting than the 8B. The same quality gap at less cost is available by running 70B at lower volume or on a paid tier.

For iterative pipeline development, 8B at $0.003/run is the right choice. Switch to 70B for final evaluation once generation prompts are stable. The 25% quality gap is worth paying for validation, not experimentation.
...
### Fix mismatch fit level overlap (actual: 0.60, target: < 0.20)
...
**1. Structural validation and quality labeling are different evaluations.** 100% schema pass rate is a floor. 58% quality pass rate is the actual signal. Run both.

**2. Templates that require intentional failure are the hardest to generate.** `career_changer` (64% failure) vs. `technical` (27% failure). The LLM's helpfulness bias works against you when you need bad candidates.
...
**4. Seniority mismatch is the dominant failure mode across models.** 19.4% (8B) and 16.7% (70B). Add explicit seniority instructions or it will stay the top failure mode regardless of model size.
